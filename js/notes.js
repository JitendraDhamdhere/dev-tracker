...
(function () {
  const NotesModule = {
    ...
    initEventListeners: function () {
      ...
      if (pinBtn) pinBtn.addEventListener('click', () => this.toggleActiveNoteProp('pinned'));
      if (favBtn) favBtn.addEventListener('click', () => this.toggleActiveNoteProp('favorite'));
      if (archiveBtn) archiveBtn.addEventListener('click', () => this.toggleActiveNoteProp('archived'));
      ...
      if (searchBox) searchBox.addEventListener('input', () => this.renderNotesList());
      if (filterCat) filterCat.addEventListener('change', () => this.renderNotesList());
      if (sortBy) sortBy.addEventListener('change', () => this.renderNotesList());
      ...
      if (importInput) {
        importInput.addEventListener('change', (e) => this.importNotes(e));
      }
    },
    ...
    buildListHtml: function (notesList) {
    ...
    updateNotesListSilently: function () {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (Array.isArray(importedNotes)) {
            const currentNotes = StorageService.get('notes') || [];
            ...
            StorageService.set('notes', currentNotes);
            this.render();
