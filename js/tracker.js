...
(function () {
  const TrackerModule = {
    ...
    // Java Tracker Render
    renderJavaTracker: function () {
      const container = document.getElementById('java-tracker-container');
      if (!container) return;

      const trackers = StorageService.get('trackers') || {};
    ...
    buildTrackerHeader: function (title, percent, count, total, unit = 'skills mastered') {
      return `
    ...
    // Interactions
    toggleItem: function (trackerKey, itemKey) {
      const trackers = StorageService.get('trackers') || {};
      trackers[trackerKey][itemKey] = !trackers[trackerKey][itemKey];

      StorageService.set('trackers', trackers);
      this.renderActiveTracker();
    ...
    toggleDsaCompleted: function (key) {
      const trackers = StorageService.get('trackers') || {};
    ...
    updateDsaVal: function (key, difficulty, val) {
      const trackers = StorageService.get('trackers') || {};
    ...
    syncTrackerToRoadmap: function (itemKey, isCompleted) {
      const roadmap = StorageService.get('roadmap') || {};
      ...
      if (roadmapNode) {
        ...
        StorageService.set('roadmap', roadmap);
